...
void StateMachineTypeC(Port_t *port)
{
    ...
    if (platform_get_device_irq_state(port->dev))
    {
        /* Read the interrupta, interruptb, status0, status1 and
         * interrupt registers.
         */
        DeviceRead(port->dev, regInterrupta, 5,
...
void StateMachineUnattached(Port_t *port)
{
    ...
    if (port->Registers.Status.I_TOGDONE)
    {
        ...
        DeviceRead(port->dev, regStatus1a, 1,
     ...
     (defined(FSC_HAVE_SNK) && defined(FSC_HAVE_ACCMODE)))
        ...
        default:
            ...
            DeviceWrite(port->dev, regControl2, 1,
...
void StateMachineAttachWaitSource(Port_t *port)
{
    ...
    DeviceRead(port->dev, regInterrupt, 1,
...
void StateMachineAttachedSource(Port_t *port)
{
    ...
    case 0:
        ...
        {
            platform_set_vbus_lvl_enable(port->dev, VBUS_LVL_ALL, FALSE,
                                         FALSE);
            platform_set_vbus_discharge(port->dev, TRUE);

            notify_observers(CC_NO_ORIENT, port->dev, 0);

            USBPDDisable(port, TRUE);

            /* VConn off and Pulldowns while detatching */
            port->Registers.Switches.byte[0] = 0x03;
            DeviceWrite(port->dev, regSwitches0, 1,
    ...
    case 1:
        ...
        {
            platform_set_vbus_discharge(port->dev, FALSE);
            SetStateUnattached(port);
        }
        else
        {
...
void SetStateUnattached(Port_t *port)
{
    ...
    DeviceWrite(port->dev, regMeasure, 1, &port->Registers.Measure.byte);
    ...
    if (port->Registers.Control.HOST_CUR != 0x1)
    {
        ...
        DeviceWrite(port->dev, regControl0, 1,
    ...
    port->Registers.Control.TOGGLE = 1;
    DeviceWrite(port->dev, regControl0, 3,&port->Registers.Control.byte[0]);
...
void SetStateAttachWaitSource(Port_t *port)
{
    ...
    platform_set_vbus_lvl_enable(port->dev, VBUS_LVL_ALL, FALSE, FALSE);
...
void SetStateAttachedSource(Port_t *port)
{
    ...
    /* Enable 5V VBus */
    platform_set_vbus_lvl_enable(port->dev, VBUS_LVL_5V, TRUE, TRUE);
...
void SetStateUnsupportedAccessory(Port_t *port)
{
    ...
    notify_observers(ACC_UNSUPPORTED, port->dev, 0);
}
#endif /* (defined(FSC_HAVE_SNK) && defined(FSC_HAVE_ACCMODE)) */
...
CCTermType DecodeCCTerminationSource(Port_t *port)
{
    ...
    /* Delay to allow measurement to settle */
    platform_delay_10us(25);
    DeviceRead(port->dev, regStatus0, 1,
...
FSC_BOOL isVBUSOverVoltage(Port_t *port, FSC_U16 vbus_mv)
{
    ...
    if (port->Registers.Measure.byte != measure.byte)
    {
        ...
        DeviceWrite(port->dev, regMeasure, 1, &measure.byte);
        mdacUpdated = TRUE;
        /* Delay to allow measurement to settle */
        platform_delay_10us(35);
    }

    DeviceRead(port->dev, regStatus0, 1, &val);
...
void setStateSource(Port_t *port, FSC_BOOL vconn)
{
    ...
    DeviceWrite(port->dev, regPower, 1, &port->Registers.Power.byte);
...
void clearState(Port_t *port)
{
    ...
    platform_set_pps_voltage(port->dev, 0);
    ...
    DeviceWrite(port->dev, regControl0, 3,
                &port->Registers.Control.byte[0]);
