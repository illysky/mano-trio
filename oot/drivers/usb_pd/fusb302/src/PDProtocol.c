...
void ProtocolSendCableReset(Port_t *port)
{
    ...
    DeviceWrite(port->dev, regFIFO, port->ProtocolTxBytes,
...
void ProtocolGetRxPacket(Port_t *port, FSC_BOOL HeaderReceived)
{
    ...
    DeviceRead(port->dev, regStatus1, 1,
    ...
    if (HeaderReceived == FALSE)
    {
        ...
        DeviceRead(port->dev, regFIFO, 3, &data[0]);
        ...
        (port->PolicyRxHeader.MessageType == CMTGoodCRC))
    {
        ...
        DeviceRead(port->dev, regFIFO, 4, data);
        ...
        (port->PolicyRxDataObj[0].SVDM.CommandType == 0)) /* Initiator */
    {
        ...
        DeviceRead(port->dev, regInterruptb, 3,
    ...
    else
    {
         ...
         */
        DeviceRead(port->dev, regStatus0, 2,
...
void ProtocolTransmitMessage(Port_t *port)
{
    ...
    if (DeviceWrite(port->dev, regFIFO, port->ProtocolTxBytes,
    ...
    DeviceWrite(port->dev, regControl3, 1,&port->Registers.Control.byte[3]);
    port->Registers.Control.TX_START = 1;
    DeviceWrite(port->dev, regControl0, 1,&port->Registers.Control.byte[0]);
    port->Registers.Control.TX_START = 0;
...
void ResetProtocolLayer(Port_t *port, FSC_BOOL ResetPDLogic)
{
    ...
    if (ResetPDLogic)
    {
        DeviceWrite(port->dev, regReset, 1, &data);
    }
