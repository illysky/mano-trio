...
void PortInit(Port_t *port, void* dev)
{
    FSC_U8 i;
    port->dev = dev;
...
void SetPEState(Port_t *port, PolicyState_t state)
{
    ...
    notify_observers(PD_STATE_CHANGED, port->dev, 0);
#ifdef FSC_DEBUG
...
{
    FSC_U8 reset = 0x01;
    DeviceWrite(port->dev, regReset, 1, &reset);

    DeviceRead(port->dev, regDeviceID, 1, &port->Registers.DeviceID.byte);
    DeviceRead(port->dev, regSwitches0,1,&port->Registers.Switches.byte[0]);
    DeviceRead(port->dev, regSwitches1,1,&port->Registers.Switches.byte[1]);
    DeviceRead(port->dev, regMeasure, 1, &port->Registers.Measure.byte);
    DeviceRead(port->dev, regSlice, 1, &port->Registers.Slice.byte);
    DeviceRead(port->dev, regControl0, 1, &port->Registers.Control.byte[0]);
    DeviceRead(port->dev, regControl1, 1, &port->Registers.Control.byte[1]);
    DeviceRead(port->dev, regControl2, 1, &port->Registers.Control.byte[2]);
    DeviceRead(port->dev, regControl3, 1, &port->Registers.Control.byte[3]);
    DeviceRead(port->dev, regMask, 1, &port->Registers.Mask.byte);
    DeviceRead(port->dev, regPower, 1, &port->Registers.Power.byte);
    DeviceRead(port->dev, regReset, 1, &port->Registers.Reset.byte);
    DeviceRead(port->dev, regOCPreg, 1, &port->Registers.OCPreg.byte);
    DeviceRead(port->dev, regMaska, 1, &port->Registers.MaskAdv.byte[0]);
    DeviceRead(port->dev, regMaskb, 1, &port->Registers.MaskAdv.byte[1]);
    DeviceRead(port->dev, regStatus0a, 1, &port->Registers.Status.byte[0]);
    DeviceRead(port->dev, regStatus1a, 1, &port->Registers.Status.byte[1]);
    DeviceRead(port->dev, regInterrupta, 1,&port->Registers.Status.byte[2]);
    DeviceRead(port->dev, regInterruptb, 1,&port->Registers.Status.byte[3]);
    DeviceRead(port->dev, regStatus0, 1, &port->Registers.Status.byte[4]);
    DeviceRead(port->dev, regStatus1, 1, &port->Registers.Status.byte[5]);
    DeviceRead(port->dev, regInterrupt, 1, &port->Registers.Status.byte[6]);
}

void InitializeTypeCVariables(Port_t *port)
{
    port->Registers.Mask.byte = 0xFF;
    DeviceWrite(port->dev, regMask, 1, &port->Registers.Mask.byte);
    port->Registers.MaskAdv.byte[0] = 0xFF;
    DeviceWrite(port->dev, regMaska, 1, &port->Registers.MaskAdv.byte[0]);
    port->Registers.MaskAdv.M_GCRCSENT = 1;
    DeviceWrite(port->dev, regMaskb, 1, &port->Registers.MaskAdv.byte[1]);

    /* Enable interrupt Pin */
    port->Registers.Control.INT_MASK = 0;
    DeviceWrite(port->dev, regControl0, 1,&port->Registers.Control.byte[0]);
    ...
    DeviceWrite(port->dev, regControl2, 1,&port->Registers.Control.byte[2]);
    port->Registers.Control4.TOG_USRC_EXIT = 0;
    DeviceWrite(port->dev, regControl4, 1, &port->Registers.Control4.byte);
